  ...
  _buildFilename(transcriptionId, timestamp) {
    if (timestamp) {
      ...
      if (!isNaN(d.getTime())) {
        ...
        return `Mnemora-${date}-${time}-${transcriptionId}.webm`;
      }
    }
    return `Mnemora-${transcriptionId}.webm`;
  }
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (stats.mtimeMs < cutoffMs) {
            fs.unlinkSync(filePath);
            // The final dash-delimited token is the id for both Mnemora and legacy files.
