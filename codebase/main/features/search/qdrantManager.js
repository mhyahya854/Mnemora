...
const {
  ...
  resolveBinaryPath,
  gracefulStopProcess,
} = require("../../infrastructure/runtime/serverUtils");
...
const CACHE_ROOT = path.join(os.homedir(), ".cache", "mnemora");
const STORAGE_DIR = path.join(CACHE_ROOT, "qdrant-data");
const LEGACY_STORAGE_DIR = path.join(os.homedir(), ".cache", "openwhispr", "qdrant-data");

function migrateLegacyStorage() {
  if (fs.existsSync(STORAGE_DIR) || !fs.existsSync(LEGACY_STORAGE_DIR)) return;
  fs.mkdirSync(CACHE_ROOT, { recursive: true });
  try {
    fs.renameSync(LEGACY_STORAGE_DIR, STORAGE_DIR);
    debugLogger.info("Migrated legacy semantic index storage", { from: LEGACY_STORAGE_DIR });
  } catch (error) {
    debugLogger.warn("Could not migrate legacy semantic index; it can be rebuilt", {
      error: error.message,
    });
  }
}
