...
const { createImportSignal, importLocalFile } = require("../transcription/downloadUtils");
const { resolveBinaryPath, gracefulStopProcess } = require("../../infrastructure/runtime/serverUtils");
const { getModelsDirForService } = require("../transcription/modelDirUtils");
const { convertToWav } = require("../transcription/ffmpegUtils");
...
const sidecarPidFile = require("../../infrastructure/runtime/sidecarPidFile");
const {
  transcriptsOverlap,
...
class DiarizationManager {
  ...
  isAvailable() {
    return this.getBinaryPath() !== null && this.isModelInstalled();
  }
  ...
  async importModels(progressCallback = null) {
    const modelsDir = this.getModelsDir();
    await fsPromises.mkdir(modelsDir, { recursive: true });

    const modelsReady = this.isModelInstalled();
    const vadReady = this.isVadModelInstalled();
    ...
    try {
      ...
      await importLocalFile(modelsDir, {
        title: "Select Diarization Models Package (Zip / Tar)",
        filters: [{ name: "Archive Files (*.zip, *.tar, *.tar.bz2)", extensions: ["zip", "tar", "tar.bz2", "bz2"] }],
        isArchive: true,
        extractDir: modelsDir,
        expectedFiles: [SEGMENTATION_ONNX, EMBEDDING_ONNX, SILERO_VAD_ONNX].map((file) =>
        ...
        onProgress: (importedBytes, totalBytes) => {
          if (progressCallback) {
            progressCallback({
              type: "progress",
              stage: "extracting",
              percentage: totalBytes > 0 ? Math.round((importedBytes / totalBytes) * 100) : 50,
            });
          }
        }
      });

      if (!this.isModelInstalled() || !this.isVadModelInstalled()) {
        throw new Error("Imported package is incomplete: missing required diarization model files.");
      }
      ...
      debugLogger.info("Diarization models imported", { modelsDir });
      return { success: true, path: modelsDir };
    } catch (error) {
  ...
  async diarize(wavPath, options = {}) {
    ...
    if (!this.isModelInstalled()) {
      debugLogger.warn("Diarization models are not installed");
      return [];
    }
