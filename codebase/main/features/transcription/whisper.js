...
const modelRegistryData = require("./modelRegistryData.json");
...
function shouldRewarmOnWake({ useCuda, modelName, transcribing, rewarmInFlight }) {
  // Only re-warm a running CUDA whisper-server: sleep evicts its model from VRAM.
  return !!useCuda && !!modelName && !transcribing && !rewarmInFlight;
}

class WhisperManager {
  constructor() {
    this.cachedFFmpegPath = null;
    this.currentImportProcess = null;
  ...
  getModelPath(modelName) {
    ...
    const localPath = path.join(this.getModelsDir(), config.fileName);
    if (fs.existsSync(localPath)) return localPath;

    const bundledPaths = [];
    if (process.resourcesPath) {
      bundledPaths.push(path.join(process.resourcesPath, "bin", "whisper-models", config.fileName));
    }
    bundledPaths.push(
      path.join(__dirname, "..", "..", "..", "resources", "bin", "whisper-models", config.fileName)
    );
    return bundledPaths.find((candidate) => fs.existsSync(candidate)) || localPath;
  }
  ...
  async initializeAtStartup(settings = {}) {
    ...
    try {
      ...
      await cleanupStaleImports(this.getModelsDir());

      // Pre-warm whisper-server if local mode enabled (eliminates 2-5s cold-start delay)
      const { whisperModel, useCuda } = settings;

      if (whisperModel && this.serverManager.isAvailable()) {
        ...
        if (fs.existsSync(modelPath)) {
          ...
          debugLogger.debug("Skipping server pre-warm: local model is unavailable", {
            model: whisperModel,
            modelPath,
          });
        }
        ...
        debugLogger.debug("Skipping server pre-warm", {
          reason: !whisperModel ? "no model selected" : "server binary not available",
        });
      }
    } catch (error) {
  ...
  async logDependencyStatus() {
    ...
    debugLogger.info("Mnemora dependency check", status);
  ...
  async startServer(modelName, options = {}) {
    ...
    const modelPath = this.getModelPath(modelName);
    if (!fs.existsSync(modelPath)) {
      return { success: false, reason: `Local model "${modelName}" is unavailable` };
    }
  ...
  async transcribeLocalWhisper(audioBlob, options = {}) {
    ...
    if (!fs.existsSync(modelPath)) {
      throw new Error(`Whisper model "${model}" is unavailable. Import it from Models settings.`);
    }
  ...
  async importWhisperModel(modelName, progressCallback = null) {
    this.validateModelName(modelName);
    ...
    if (!spaceCheck.ok) {
      throw new Error(
        `Not enough disk space to import model. Need ~${Math.round((modelConfig.size * 1.2) / 1_000_000)}MB, ` +
          `only ${Math.round(spaceCheck.availableBytes / 1_000_000)}MB available.`
      );
    }

    const { signal, abort } = createImportSignal();
    this.currentImportProcess = { abort };

    try {
      await importLocalFile(modelPath, {
        title: `Select Whisper ${modelName} model file (ggml-${modelName}.bin)`,
        ...
        onProgress: (importedBytes, totalBytes) => {
          if (progressCallback) {
            progressCallback({
              ...
              percentage: totalBytes > 0 ? Math.round((importedBytes / totalBytes) * 100) : 0,
            });
          }
        },
      });
  ...
  async checkModelStatus(modelName) {
    ...
    return { model: modelName, installed: false, success: true };
  }
