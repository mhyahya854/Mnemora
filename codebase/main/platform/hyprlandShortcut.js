...
const DBUS_SERVICE_NAME = "com.mnemora.App";
const DBUS_OBJECT_PATH = "/com/mnemora/App";
const DBUS_INTERFACE = "com.mnemora.App";
...
const BINDS_FILENAME = "mnemora-binds.conf";
const LEGACY_BINDS_FILENAME = "openwhispr-binds.conf";
const MANAGED_HEADER_LINES = [
  "# Mnemora keybinds (managed automatically)",
...
class HyprlandShortcutManager {
  ...
  _ensureSourceInMainConfig() {
    ...
    const legacySource = `./${LEGACY_BINDS_FILENAME}`;
    if (content.includes(legacySource)) {
      content = content
        .split("\n")
        .filter((line) => !line.includes(legacySource))
        .join("\n");
      fs.writeFileSync(mainConfig, content, "utf-8");
      fs.rmSync(path.join(getHyprConfigDir(), LEGACY_BINDS_FILENAME), { force: true });
    }
   ...
   *
   * Also writes the bind to mnemora-binds.conf (sourced from hyprland.conf)
  ...
  async registerKeybinding(hotkey) {
    ...
    try {
      // First unregister any existing Mnemora binding if the hotkey changed.
