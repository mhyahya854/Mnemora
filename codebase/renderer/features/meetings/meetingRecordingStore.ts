...
import type { SystemAudioAccessResult, SystemAudioStrategy } from "../../shared/types/electron";
import {
  DEFAULT_SYSTEM_AUDIO_ACCESS,
  getDisplayCaptureModeForStrategy,
  getFallbackSystemAudioAccess,
  isRendererSystemAudioStrategy,
} from "./systemAudioAccess";
import {
  DEFAULT_EXPECTED_SPEAKER_COUNT,
  MAX_SPEAKER_COUNT,
} from "../../../shared/config/speakerDetection.json";
...
const getMeetingTranscriptionOptions = () => {
  ...
  return {
    ...
    localProvider: "whisper" as const,
    localModel: state.whisperModel || "base",
    language,
  };
};
...
// Detach the AudioContext from hardware output Ã¢â‚¬â€ when BT headphones switch to
...
// Pipeline lives in module scope Ã¢â‚¬â€ not on React refs Ã¢â‚¬â€ so it survives
...
export async function prepareTranscription(): Promise<void> {
  ...
  logger.info("Preparing local meeting transcription", {}, "meeting");
  preparePromise = (async () => {
    try {
      ...
      if (result?.success) {
        ...
        logger.warn("Meeting transcription warmup failed", { error: result.error }, "meeting");
      }
    } catch (error) {
      logger.warn(
        "Meeting transcription warmup failed",
...
export async function startRecording(args: StartRecordingArgs): Promise<void> {
  ...
  isStartingFlag = true;
  const initialEnabled = args.diarizationEnabled ?? getSettings().speakerDiarizationEnabled ?? true;
      ...
      (data: {
        ...
        if (data.type === "partial") {
          if (data.source === "mic") {
            ...
            if (!systemPartialSpeakerIdValue) {
              // Reuse the recent system speaker before minting Ã¢â‚¬â€ the partial id is
...
// Throttled resize listener Ã¢â‚¬â€ keeps layout reflows during drag from thrashing
