...
if (!swiftTarget) {
  console.error(`[mic-listener] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..", "..");
const swiftSource = path.join(projectRoot, "native", "helpers", "macos", "macos-mic-listener.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-mic-listener");
const hashFile = path.join(outputDir, `.macos-mic-listener.${targetArch}.hash`);
...
function log(message) {
  console.log(`[mic-listener] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[mic-listener] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[mic-listener] Failed to compile macOS mic listener binary.");
...
try {
  ...
  console.warn(`[mic-listener] Unable to set executable permissions: ${error.message}`);
}

// Verify the compiled binary matches the target architecture
if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[mic-listener] FATAL: Compiled binary architecture does not match target (${targetArch}). ` +
...
log(`Successfully built macOS mic listener binary (${targetArch}).`);
