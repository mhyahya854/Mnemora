...
if (!swiftTarget) {
  console.error(`[media-remote] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..", "..");
const swiftSource = path.join(projectRoot, "native", "helpers", "macos", "macos-media-remote.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-media-remote");
const hashFile = path.join(outputDir, `.macos-media-remote.${targetArch}.hash`);
...
function log(message) {
  console.log(`[media-remote] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[media-remote] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[media-remote] Failed to compile macOS media-remote binary.");
...
try {
  ...
  console.warn(`[media-remote] Unable to set executable permissions: ${error.message}`);
}

if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[media-remote] FATAL: Compiled binary architecture does not match target (${targetArch}).`
  );
  process.exit(1);
}
...
log(`Successfully built macOS media-remote binary (${targetArch}).`);
