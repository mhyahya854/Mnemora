...
## 2026-08-06 final repository reconciliation - wording supersession

The 2026-08-05 continuation record above used the phrase "exactly equals the pre-audit baseline" for the full-tree manifests. Precise reconciliation shows that the two full-tree manifest FILES are not byte-identical: `AUDIT_CODEBASE_BASELINE_SHA256.txt` SHA-256 `201FC0BDAE689102ED823385C8F8EFEAB1A3739C65EAB8743398DFE94F8D0006` versus `AUDIT_CODEBASE_FINAL_SHA256.txt` SHA-256 `6AA22642525633E6E59E51FB1EB281D4AAE78602B15FAF7744E7FF9B1EF4CF6C` (same 2,991,504 bytes and 24,839 lines; the baseline uses filesystem-traversal order with backslash separators, the final manifest uses sorted order with forward slashes). Their NORMALIZED path-to-SHA-256 mappings are identical (0 added, 0 missing, 0 changed, 0 duplicates, 0 malformed). The authoritative current record is `FINAL-REPOSITORY-RECONCILIATION.md`/`.json`, which distinguishes raw-file equality from normalized-mapping equality and separates tracked Git parity, Git LFS verification, and local-only inventory claims. This section supersedes the loose wording only; the historical evidence itself is preserved.
